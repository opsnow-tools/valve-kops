...
read_kops_config() {
    if [ ! -z ${KOPS_CLUSTER_NAME} ]; then
        _command "aws s3 ls s3://${KOPS_STATE_STORE} | grep ${KOPS_CLUSTER_NAME}.valve-kops | wc -l | xargs"
        COUNT=$(aws s3 ls s3://${KOPS_STATE_STORE} | grep ${KOPS_CLUSTER_NAME}.valve-kops | wc -l | xargs)

        if [ "x${COUNT}" != "x0" ]; then
            CONFIG=${SHELL_DIR}/build/${THIS_NAME}/kops-config.sh

            _command "aws s3 cp s3://${KOPS_STATE_STORE}/${KOPS_CLUSTER_NAME}.valve-kops ${CONFIG}"
            aws s3 cp s3://${KOPS_STATE_STORE}/${KOPS_CLUSTER_NAME}.valve-kops ${CONFIG} --quiet
...
save_kops_config() {
    ...
    if [ ! -z ${S3_SYNC} ] && [ ! -z ${KOPS_CLUSTER_NAME} ]; then
        _command "aws s3 cp ${CONFIG} s3://${KOPS_STATE_STORE}/${KOPS_CLUSTER_NAME}.valve-kops"
        aws s3 cp ${CONFIG} s3://${KOPS_STATE_STORE}/${KOPS_CLUSTER_NAME}.valve-kops --quiet
    fi
}
...
kops_create() {
    ...
    echo "    --ssh-public-key=~/.ssh/id_rsa.pub" >> ${KOPS_CREATE}
